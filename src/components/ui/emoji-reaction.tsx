...
const DEFAULT_EMOJI_DATA: EmojiData = {
  ...
  emojis: {
    ...
    'grinning-face-with-smiling-eyes': 'grinning-face-with-smiling-eyes_1f604.png',
  },
}

const DEFAULT_EMOJIS = Object.keys(DEFAULT_EMOJI_DATA.emojis)

const SURFACE = 'bg-stage border border-edge'
...
function getPlacement(trigger: DOMRect, width: number, height: number, align: Align): Placement {
  const anchored =
    align === 'left' ? trigger.left : align === 'right' ? trigger.right - width : trigger.left + trigger.width / 2 - width / 2
...
function makeParticles(name: string, seed: number, from: DOMRect, bar: DOMRect): Particle[] {
...
const BurstEmoji = memo(function BurstEmoji({
  ...
  return (
    <motion.span
      ...
      style={{ left: particle.originX, top: particle.originY, marginLeft: -size / 2, marginTop: -size / 2 }}
    ...
    >
      <Emoji name={particle.name} width={size} height={size} draggable={false} className="max-w-none" />
    </motion.span>
  )
})
...
export function EmojiReaction({
  ...
  const [placement, setPlacement] = useState<Placement>({ side: 'top', shift: 0, tailX: 0 })
  ...
  const burst = particles.map((particle) => <BurstEmoji key={particle.id} particle={particle} size={s.burst} onDone={settle} />)
    ...
    <EmojiProvider data={emojiData}>
      ...
      >
        <AnimatePresence>
          {open && (
            <motion.div
              className={cn('absolute z-30', anchor, top ? 'bottom-full mb-3' : 'top-full mt-3')}
            ...
            >
              <div
                ...
                className={cn('relative flex items-center rounded-full shadow-xl', SURFACE, s.pill)}
              >
                {burst}
                ...
                {emojis.map((name, i) => (
                  <motion.button
                    ...
                    onPointerDown={(event) => startHold(name, event.currentTarget.getBoundingClientRect())}
                    onPointerUp={stopHold}
                    onPointerLeave={stopHold}
                    onPointerCancel={stopHold}
                    onClick={(event) => event.detail === 0 && react(name, event.currentTarget.getBoundingClientRect())}
                    className="relative z-10 rounded-full p-1 outline-none focus-visible:ring-2 focus-visible:ring-back"
                    ...
                    whileTap={{ scale: 0.92 }}
                  >
                    <Emoji name={name} width={s.emoji} height={s.emoji} draggable={false} className="max-w-none" />
                  </motion.button>
                ))}
              </div>

              <span
                className={cn('absolute size-3 -translate-x-1/2 rounded-full', SURFACE, top ? '-bottom-1' : '-top-1')}
                style={{ left: placement.tailX }}
              />
              <span
                className={cn('absolute size-1.5 -translate-x-1/2 rounded-full', SURFACE, top ? '-bottom-4' : '-top-4')}
        ...
        <Trigger
          ...
          className={
            asChild
              ? undefined
              : cn(
                  'relative z-10 grid place-items-center rounded-full text-paper/70 backdrop-blur transition-colors hover:text-paper focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-back',
                  SURFACE,
                  s.trigger,
                )
          }
        >
          {asChild ? children : open ? (
            <X className={s.icon} strokeWidth={2} />
          ) : last ? (
            <Emoji name={last} width={s.emoji * 0.72} height={s.emoji * 0.72} draggable={false} className="max-w-none" />
          ) : (
