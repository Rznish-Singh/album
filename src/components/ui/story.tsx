...
export const Story = React.forwardRef<HTMLDivElement, StoryProps>(
  ({ mediaLength, duration = 2000, className, children, ...props }, ref) => {
    ...
    if (mediaLength === 0) {
      return <div className="text-center text-haze">No stories to display</div>
    }
    ...
    <div className={cn('flex space-x-1', className)} ref={ref} {...props}>
      {Array.from({ length: mediaLength }).map((_, index) => {
        ...
        return (
          <div
            key={index}
            className={cn('h-1 flex-1 cursor-pointer rounded bg-edge transition-colors hover:bg-haze/60', progressWrapClass)}
          ...
          >
            <div
              className={cn(
                ...
                isActive ? 'bg-key' : isCompleted ? 'bg-key' : 'bg-transparent',
                progressActiveClass,
              )}
...
export const StorySlide = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { index: number }>(
...
export const StoryControls: React.FC<ButtonProps> = ({ className, ...props }) => {
  const { isPaused, isEnded, handleControl } = useStoryContext()
  return (
    <Button onClick={handleControl} size="icon" {...props} className={className}>
